...
- A workflow dispatch immediately after push once built the preceding commit. When publishing, pass expected_revision with the intended full SHA, verify the run headSha at start, and check the anonymous registry config revision after publication. A green build alone is not sufficient proof that the intended fix was published.
